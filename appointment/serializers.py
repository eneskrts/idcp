...
class AvailabilitySerializer(serializers.ModelSerializer):    
    ...
    def check_date(self,validated_data):
        ...
        if (time.upper.year != time.lower.year) or (time.upper.month != time.lower.month) or (time.upper.day != time.lower.day):
...
class MeetingRoomSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        try:
            time = datetime.datetime.strptime(validated_data["meeting_start_time"],'%Y-%m-%dT%H:%M:%S').astimezone(tz=ZoneInfo("Turkey"))
            validated_data["meeting_start_time"] = time
        except:
            error={
                'messages':'Tarih "1999-09-09 12:00:00" formatında olmalıdır','start_time':validated_data["meeting_start_time"]
            }
            raise serializers.ValidationError(error)
