...
def CheckAvailability(meeting_times, attendees):
    ...
    for hour in hours:
        available_users = Availability.objects.filter(available_user__in=attendees,
                                                      available_time__contained_by=
                                                      DateTimeTZRange(hour, hour + datetime.timedelta(hours=1)),
                                                      reserved=0)
...
def UpdateAppointmentRequest(appointment_request, answer):
    ...
    availability = Availability.objects.get(available_user=appointment_request.doctor,
                                            available_time=DateTimeTZRange(time, time + datetime.timedelta(hours=1)))
    if answer == 'accepted':
        availability.reserved = 1

        # Kabul edilen istek toplantısı ile aynı saattaki istekler reddediliyor
        all_requests = AppointmentRequest.objects.filter(meeting__meeting_start_time=time,
...
class AvailableFilter(FilterSet):
    ...
    searching_timezone = filters.CharFilter(label="timezone")
    ...
    def filter_queryset(self, queryset):
        if self.data:
            ...
            if self.data['available_time_max'] and self.data['available_time_min']:
                available_time_max = datetime.datetime.strptime(self.data['available_time_max'],
                                                                '%Y-%m-%dT%H:%M').astimezone(tz=ZoneInfo("Turkey"))
                available_time_min = datetime.datetime.strptime(self.data['available_time_min'],
                                                                '%Y-%m-%dT%H:%M').astimezone(tz=ZoneInfo("Turkey"))
